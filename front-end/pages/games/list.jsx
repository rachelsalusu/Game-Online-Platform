    ...
    for (const i in gameListState.gameList) {
      elements.push(
        <Card style={{ width: '18rem' }} className="m-3" key={i}>
          ...
          <CardBody>
            <CardTitle tag="h5">
              <Link href={sessionStorage.userData ? '/games/' + gameListState.gameList[i].id : '/login'} className={styles.removeHighlight}>
  ...
  return (
    <>
      ...
      <Container className={styles.container}>
        <p className="d-flex justify-content-center h1 text-light">List of Available Games</p>
        ...
        <Container className="d-flex justify-content-center align-items-center mt-2">{createElementGameList()}</Container>
      </Container>
    </>
  );
}
